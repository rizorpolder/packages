	...
	public class ManagerAudio : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			for (var i = dynamicSettingsListenerList.Count - 1; i > -1; i--)
				...
				{
					if (audioSourcesVolumeChangeList.All(a =>
						    a.audioSource != dynamicSettingsListenerList[i].audioSource))
					{
						var dynamicSettings = dynamicSettingsListenerList[i];
						...
						{
							audioSource.pitch = Random.Range(settingsAudioInstance.pitchMin,
								settingsAudioInstance.pitchMax);
		...
		private void PlayAudioClipProcess(AudioSource audioSource,
			SettingsAudioInstance settingsAudioInstance,
			Settings settings,
			float delayExtra)
		{
			audioSource.outputAudioMixerGroup = settingsAudioInstance.audioMixer;
